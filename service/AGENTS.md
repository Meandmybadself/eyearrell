...
Set `DATABASE_URL`, `SESSION_SECRET`, `CLIENT_URL`, `SERVICE_PUBLIC_URL`, `AWS_SES_SMTP_HOST`, `AWS_SES_SMTP_USER`, `AWS_SES_SMTP_PASS`, `AWS_SES_FROM_EMAIL`, and `AWS_SES_FROM_NAME` in `.env` before running the app. Never commit secrets. Use `npx prisma migrate dev --name <change>` after editing `schema.prisma`, and check in the generated migration and updated client.
